...
#define MAX_SLOTS 4
#define MAX_AMOUNT 32765 ////Max amount of a single stacked item
...
struct item {
	int id;
	t_itemid nameid;			//道具id
	short amount;				//数量
	unsigned int equip; // location(s) where item is equipped (using enum equip_pos for bitmasking)装备位置
	char identify;		//鉴定
	char refine;		//精炼
	char attribute;					//损坏	
	t_itemid card[MAX_SLOTS];			//卡片
	struct s_item_randomoption option[MAX_ITEM_RDM_OPT];		// max of 5 random options can be supported.词条
	unsigned int expire_time;				//过期时间戳
	char favorite, bound;				//收藏，绑定
	uint64 unique_id;					//唯一ID
	unsigned int equipSwitch; // location(s) where item is equipped for equip switching (using enum equip_pos for bitmasking)切换装备位置
	uint8 enchantgrade;			//评级
} __attribute__((packed));
...
struct guild {
	int guild_id;														//工会ID
	short guild_lv, connect_member, max_member, average_lv;				//工会等级,公会在线人数，工会最大人数，工会平均等级
	t_exp exp;															//工会经验值
	t_exp next_exp;														//下级经验值
	int skill_point;													//剩余工会技能点数
	char name[NAME_LENGTH],master[NAME_LENGTH];							//工会名称，会长名称
	struct guild_member member[MAX_GUILD];								//工会成员
	struct guild_position position[MAX_GUILDPOSITION];					//驻地？
	char mes1[MAX_GUILDMES1],mes2[MAX_GUILDMES2];
	int emblem_len,emblem_id;											//会标长度，会标ID
	char emblem_data[2048];												//会标数据
	struct guild_alliance alliance[MAX_GUILDALLIANCE];					//结盟？
	struct guild_expulsion expulsion[MAX_GUILDEXPULSION];				//驱逐？
	struct guild_skill skill[MAX_GUILDSKILL];							//技能
	struct Channel *channel;											//频道
	int instance_id;													//副本ID
	time_t last_leader_change;											//上次更换领导人时间		
