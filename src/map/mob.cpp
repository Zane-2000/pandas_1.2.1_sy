...
/*==========================================
 * Generates the basic mob data using the spawn_data provided. 使用提供的spawn_data生成基本暴徒数据。
 *------------------------------------------*/
...
#else
struct mob_data* mob_once_spawn_sub(struct block_list* bl, int16 m, int16 x, int16 y, const char* mobname, int mob_id, const char* event, unsigned int size, enum mob_ai ai, uint16 spawn_flag)
...
#else
int mob_once_spawn(map_session_data* sd, int16 m, int16 x, int16 y, const char* mobname, int mob_id, int amount, const char* event, unsigned int size, enum mob_ai ai, uint16 spawn_flag)
...
#else
		md = mob_once_spawn_sub((sd) ? &sd->bl : NULL, m, x, y, mobname, c, event, size, ai, spawn_flag);
#endif // Pandas_FuncDefine_Mob_Once_Spawn
...
static bool mob_ai_sub_hard(struct mob_data *md, t_tick tick)
{
	...
	if (!tbl) { //No targets available. 没有可用的目标。
	...
	{	//Loot time.
		...
		{	//Still not within loot range. 仍然不在战利品范围内。
			if (!(mode&MD_CANMOVE))
			{	//A looter that can't move? Real smart.  一个不会动的抢劫者？真聪明。
				mob_unlocktarget(md, tick);
				return true;
			}
			if (!can_move) //Stuck. Wait before walking. 卡住了。走路前要等一等。
				return true;
			md->state.skillstate = MSS_LOOT;
			if (!unit_walktobl(&md->bl, tbl, 0, 0))
				mob_unlocktarget(md, tick); //Can't loot... 不能抢劫。。。
			return true;
		}
		//Within looting range. 在拾取范围内。
		if (md->ud.attacktimer != INVALID_TIMER)
			return true; //Busy attacking?/忙于进攻？
		...
		//------------------------魔物掠夺道具事件----------------------------
		{	
			mapreg_setreg(add_str("$@mob_lootitems_itemid"), fitem->item.nameid);									//掠夺的道具ID
			mapreg_setreg(add_str("$@mob_lootitems_itemcount"), fitem->item.amount);								//道具数量
			mapreg_setreg(add_str("$@mob_lootitems_dropmobid"), md->db->id);										//道具由那只魔物掉落的(GID)，不是魔物掉落的时候是0
			mapreg_setreg(add_str("$@mob_lootitems_mobid"), md->db->id);											//拾取道具的魔物ID
			mapreg_setreg(add_str("$@mob_lootitems_mobgid"), md->bl.id);											//拾取道具的魔物GID
			mapreg_setregstr(add_str("$@mob_lootitems_mapname$"), (md && md->bl.m >= 0 ? map[md->bl.m].name : ""));	//地图名称
			mapreg_setreg(add_str("$@mob_lootitems_stack"), 0);														//0,官方拾取规则，1叠加拾取，2拾取后删除地面道具但不加入背包

			npc_event_doall(script_config.sy_moblootitems);



			int syi,t;
			t = mapreg_readreg(add_str("$@mob_lootitems_stack"));
			if (itemdb_isstackable2(itemdb_search(fitem->item.nameid))
				&& fitem->item.expire_time == 0
				&& t == 1
				) {
				for (syi = 0; syi < md->lootitem_count; syi++) {
					if (md->lootitems[syi].item.nameid == fitem->item.nameid
						&& md->lootitems[syi].item.bound == fitem->item.bound
						&& memcmp(&md->lootitems[syi].item.card, &fitem->item.card, sizeof(fitem->item.card)) == 0
						&& memcmp(&md->lootitems[syi].item.option, &fitem->item.option, sizeof(fitem->item.option)) == 0
						&& md->lootitems[syi].item.enchantgrade == fitem->item.enchantgrade
						&& md->lootitems[syi].item.refine == fitem->item.refine
						) {
						md->lootitems[syi].item.amount += fitem->item.amount;
						break;
					}
				}
				if (syi >= LOOTITEM_SIZE) {
...
#endif // Pandas_FuncDefine_UnitDead_With_ExtendInfo
{
	...
	if( !rebirth ) {
		...
		unit_stop_walking(&md->bl, USW_FORCE_STOP);
...
#else
		mvptomb_create(md, mvp_sd ? mvp_sd->status.name : NULL, time(NULL), mvp_sd ? mvp_sd->bl.id : 0);
#endif // Pandas_FuncParams_Mob_MvpTomb_Create
...
void mob_heal(struct mob_data *md,unsigned int heal)
{
	...
	if (battle_config.monster_hp_bars_info && !map_getmapflag(md->bl.m, MF_HIDEMOBHPBAR)) {
		...
		for(i = 0; i < DAMAGELOG_SIZE; i++)// must show hp bar to all char who already hit the mob.向所有已经攻击暴徒的角色显示hp血条
			if( md->dmglog[i].id ) {
				map_session_data *sd = map_charid2sd(md->dmglog[i].id);
				if( sd && check_distance_bl(&md->bl, &sd->bl, AREA_SIZE) ) // check if in range 检查是否在范围内
					clif_monster_hp_bar(md, sd->fd);
			}
	}
#endif
		...
		if (ms[i]->cond1 == event)
			...
			switch (ms[i]->cond1)
			{
				...
				case MSC_FRIENDHPLTMAXRATE:	// friend HP < maxhp%
					flag = ((fbl = mob_getfriendhprate(md, 0, (int)ms[i]->cond2)) != NULL); break;
				case MSC_FRIENDHPINRATE	:
					flag = ((fbl = mob_getfriendhprate(md, (int)ms[i]->cond2, ms[i]->val[0])) != NULL); break;
				case MSC_FRIENDSTATUSON:	// friend status[num] on
				case MSC_FRIENDSTATUSOFF:	// friend status[num] off
					flag = ((fmd = mob_getfriendstatus(md, ms[i]->cond1, (int)ms[i]->cond2)) != NULL); break;
				...
				case MSC_MASTERHPLTMAXRATE:
					flag = ((fbl = mob_getmasterhpltmaxrate(md, (int)ms[i]->cond2)) != NULL); break;
...
uint64 MobDatabase::parseBodyNode(const ryml::NodeRef& node) {
	...
	if (this->nodeExists(node, "Resistance")) {
		...
		if (!this->asUInt32(node, "Resistance", res))
			return 0;
	...
	if (this->nodeExists(node, "MagicResistance")) {
		...
		if (!this->asUInt32(node, "MagicResistance", mres))
			return 0;
	...
	//------------------761355061----------新增的魔物属性---------------------------
	//		Sy_addattackharm		魔物伤害增加
	// 		Sy_addskillharm			魔物技能伤害增加
	// 		Sy_subattackharm		魔物受到伤害降低
	// 		Sy_subskillharm			魔物受到技能伤害降低
	//
	// 
		//		魔物攻击伤害增加值
	if (this->nodeExists(node, "Sy_addattackharm")) {
		uint32 addattackharm;

		if (!this->asUInt32Rate(node, "Sy_addattackharm", addattackharm, UINT32_MAX))
			return 0;

		mob->sy_data.sy_addattackharm = addattackharm;
	}
	else {
		if (!exists)
			mob->sy_data.sy_addattackharm = 0;
	}

	//		魔物技能伤害增加值
	if (this->nodeExists(node, "Sy_addskillharm")) {
		uint32 addskillharm;

		if (!this->asUInt32Rate(node, "Sy_addskillharm", addskillharm, UINT32_MAX))
			return 0;

		mob->sy_data.sy_addskillharm = addskillharm;
	}
	else {
		if (!exists)
			mob->sy_data.sy_addskillharm = 0;
	}


	//		魔物受到伤害降低值
	if (this->nodeExists(node, "Sy_subattackharm")) {
		uint32 subattackharm;

		if (!this->asUInt32Rate(node, "Sy_subattackharm", subattackharm, UINT32_MAX))
			return 0;

		mob->sy_data.sy_subattackharm = subattackharm;
	}
	else {
		if (!exists)
			mob->sy_data.sy_subattackharm = 0;
	}

	//		魔物受到技能伤害降低值
	if (this->nodeExists(node, "Sy_subskillharm")) {
		uint32 subskillharm;

		if (!this->asUInt32Rate(node, "Sy_subskillharm", subskillharm, UINT32_MAX))
			return 0;

		mob->sy_data.sy_subskillharm = subskillharm;
	}
	else {
		if (!exists)
			mob->sy_data.sy_subskillharm = 0;
	}


	//		魔物减伤机制------
	if (this->nodeExists(node, "Sy_harm_on")) {
		uint32 sy_harm;

		if (!this->asUInt32Rate(node, "Sy_harm_on", sy_harm, 1))
			return 0;

		mob->sy_data.sy_harm[0] = sy_harm;
	}
	else {
		if (!exists)
			mob->sy_data.sy_harm[0] = 0;
	}


	if (this->nodeExists(node, "Sy_harm_1W_10")) {
		uint32 sy_harm;

		if (!this->asUInt32Rate(node, "Sy_harm_1W_10", sy_harm, INT_MAX))
			return 0;

		mob->sy_data.sy_harm[1] = sy_harm;
	}
	else {
		if (!exists)
			mob->sy_data.sy_harm[1] = 0;
	}

	if (this->nodeExists(node, "Sy_harm_10W_100")) {
		uint32 sy_harm;

		if (!this->asUInt32Rate(node, "Sy_harm_10W_100", sy_harm, INT_MAX))
			return 0;

		mob->sy_data.sy_harm[2] = sy_harm;
	}
	else {
		if (!exists)
			mob->sy_data.sy_harm[2] = 0;
	}

	if (this->nodeExists(node, "Sy_harm_100W_1K")) {
		uint32 sy_harm;

		if (!this->asUInt32Rate(node, "Sy_harm_100W_1K", sy_harm, INT_MAX))
			return 0;

		mob->sy_data.sy_harm[3] = sy_harm;
	}
	else {
		if (!exists)
			mob->sy_data.sy_harm[3] = 0;
	}


	if (this->nodeExists(node, "Sy_harm_1KW_1W")) {
		uint32 sy_harm;

		if (!this->asUInt32Rate(node, "Sy_harm_1KW_1W", sy_harm, INT_MAX))
			return 0;

		mob->sy_data.sy_harm[4] = sy_harm;
	}
	else {
		if (!exists)
			mob->sy_data.sy_harm[4] = 0;
	}


	if (this->nodeExists(node, "Sy_harm_1E_10W")) {
		uint32 sy_harm;

		if (!this->asUInt32Rate(node, "Sy_harm_1E_10W", sy_harm, INT_MAX))
			return 0;

		mob->sy_data.sy_harm[5] = sy_harm;
	}
	else {
		if (!exists)
			mob->sy_data.sy_harm[5] = 0;
	}


	if (this->nodeExists(node, "Sy_harm_10E_100W")) {
		uint32 sy_harm;

		if (!this->asUInt32Rate(node, "Sy_harm_10E_100W", sy_harm, INT_MAX))
			return 0;

		mob->sy_data.sy_harm[6] = sy_harm;
	}
	else {
		if (!exists)
			mob->sy_data.sy_harm[6] = 0;
	}


	if (this->nodeExists(node, "Sy_harm_100E_1KW")) {
		uint32 sy_harm;

		if (!this->asUInt32Rate(node, "Sy_harm_100E_1KW", sy_harm, INT_MAX))
			return 0;

		mob->sy_data.sy_harm[7] = sy_harm;
	}
	else {
		if (!exists)
			mob->sy_data.sy_harm[7] = 0;
	}


	if (this->nodeExists(node, "Sy_harm_1KE_1E")) {
		uint32 sy_harm;

		if (!this->asUInt32Rate(node, "Sy_harm_1KE_1E", sy_harm, INT_MAX))
			return 0;

		mob->sy_data.sy_harm[8] = sy_harm;
	}
	else {
		if (!exists)
			mob->sy_data.sy_harm[8] = 0;
	}


	if (this->nodeExists(node, "Sy_harm_1WE_10E")) {
		uint32 sy_harm;

		if (!this->asUInt32Rate(node, "Sy_harm_1WE_10E", sy_harm, INT_MAX))
			return 0;

		mob->sy_data.sy_harm[9] = sy_harm;
	}
	else {
		if (!exists)
			mob->sy_data.sy_harm[9] = 0;
	}
	//------------------761355061----------新增的魔物属性---------------------------
