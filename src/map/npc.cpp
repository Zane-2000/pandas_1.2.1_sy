...
//-------------761355061---添加事件6_2-----定义为实时事件-----
		NPCX_NATURAL_HEAL,				// sy_natural_heal	// OnPCNaturalHeal							// 当角色自动恢复时触发的事件
		NPCX_TRADE_TRADEREQUEST,		//sy_trade_traderequest	//	OnPCtraderequest					// 当玩家受到交易请求时触发的事件
...
//-------------761355061---添加事件6_1-----定义为标准事件-----
		NPCF_SY_PET_MENU,	//pc_sy_pet_menu,	//On_sy_petmenuFilter				//	当玩家点击宠物菜单时触发的实时事件
		NPCX_USE_CARD,		// sy_use_card	// OnPCUseCardFilter						// 当玩家使用卡片时触发的事件
		NPCX_LOOT_ITEMS,		//sy_moblootitems	//OnMOBlootitemsFilter					// 当魔物即将掠夺道具时触发的事件
//------------------------------------------------------------
...
const char *npc_get_script_event_name(int npce_index)
{
	...
	//-----761355061---添加事件4---------常量指向事件结构---
	case NPCF_SY_PET_MENU:
		return script_config.pc_sy_pet_menu;	//NPCF_SY_PET_MUNU,		//On_sy_petmenuFilter				//	当玩家点击宠物菜单时触发的实时事件
	case NPCX_NATURAL_HEAL:
		return script_config.sy_natural_heal;	// OnPCNaturalHeal				// 当角色自动恢复时触发的事件
	case NPCX_USE_CARD:
		return script_config.sy_use_card;	// OnPCUseCardFilter				// 当玩家使用卡片时触发的事件
	case NPCX_LOOT_ITEMS:
		return script_config.sy_moblootitems;	// OnMOBlootitemsFilter				// 当魔物即将掠夺道具时触发的事件
	case NPCX_TRADE_TRADEREQUEST:
		return script_config.sy_trade_traderequest;	// OnPCtraderequest				// 当玩家受到交易请求时触发的事件
	case NPCX_TRADE_TRADEOK:
		return script_config.sy_trade_tradeok;	// OnPCtradeok				// 当交易玩家点击确定时触发的事件
	case NPCX_TRADE_TRADECOMMIT:
		return script_config.sy_trade_tradecommit;	// OnPCtradercommit				// 当交易玩家点击交易时触发的事件
