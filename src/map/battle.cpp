...
static void battle_calc_defense_reduction(struct Damage* wd, struct block_list *src,struct block_list *target, uint16 skill_id, uint16 skill_lv)
{
	...
	if (tsc) {
		if (tsc->getSCE(SC_FORCEOFVANGUARD)) {
			pec_short i = 2 * tsc->getSCE(SC_FORCEOFVANGUARD)->val1;

			def1 = (def1 * (100 + i)) / 100;
		}

		if( tsc->getSCE(SC_CAMOUFLAGE) ){
			pec_short i = 5 * tsc->getSCE(SC_CAMOUFLAGE)->val3; //5% per second
...
// ------76135561------修改mob_Res属性公式-------------------
			//	 Res的最大伤害减少官方规定为50%。
			//	这意味着需要 625 Res才能达到这个上限
			//	这个公式对超大数值伤害减少微乎其微,再次改变减伤公式[山有]

			res = cap_value(res, 1, battle_config.max_res_mres_reduction);
			wd.damage = wd.damage / res;
			wd.damage2 = wd.damage2 / res;
		}
...
//			ad.damage = ad.damage * (5000 + mres) / (5000 + 10 * mres);
			...
			mres = cap_value(mres, 1, battle_config.max_res_mres_reduction);
			ad.damage = ad.damage / mres;
...
/*==========================================
 * Battle main entry, from skill_attack 战斗主入口，从技能攻击
 *------------------------------------------
 * Credits:
	...
	if ((d.damage + d.damage2)) {
		if (bl->type == BL_MOB) {
			if (skill_id) {
				//技能伤害增加
				if (bl->type == BL_MOB) {
					if (((TBL_MOB*)bl)->sy_data.sy_addskillharm > 0) {
						d.damage += ((TBL_MOB*)bl)->sy_data.sy_addskillharm;
					}
				}
			}
			else {
				//普通攻击伤害增加
				if (((TBL_MOB*)bl)->sy_data.sy_addattackharm > 0) {
					d.damage += ((TBL_MOB*)bl)->sy_data.sy_addattackharm;
				}
			}
		}
		...
		if (target->type == BL_MOB) {
			//受到技能伤害降低
			if (skill_id) {
				if (((TBL_MOB*)target)->sy_data.sy_subskillharm > 0) {
					d.damage -= ((TBL_MOB*)target)->sy_data.sy_subskillharm;
				}
			}
			else {
				//受到普通攻击伤害降低
				if (((TBL_MOB*)target)->sy_data.sy_subattackharm > 0) {
					d.damage -= ((TBL_MOB*)target)->sy_data.sy_subattackharm;
				}
			}


			//伤害递减机制
			int64 sh = d.damage + d.damage2;
			int fz, fm = 1;
			int i = 0;
			if (((TBL_MOB*)target)->sy_data.sy_harm[0]) {
				for (; sh > 10000 && i < 10; ) {
					sh /= 10;
					i++;
					fm *= 10;
				}
				fz = ((TBL_MOB*)target)->sy_data.sy_harm[i];
				for (; i > 0;) {
					if (fz > 0 && fm > 0) {
						d.damage = d.damage * fz / fm;
						d.damage2 = d.damage2 * fz / fm;
						break;
					}
					else {
						i--;
...
// rAthena
	...
	{ "max_res_mres_reduction",             &battle_config.max_res_mres_reduction,          1,		1,      INT_MAX,		},
