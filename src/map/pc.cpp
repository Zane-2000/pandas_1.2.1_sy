	...
	switch(type){
		...
		case SP_SY_MAXASPD:
			if (sd->state.lr_flag != 2) {
				int sy_val;
				sy_val = 0 - 10 * val;
				if (sd->bonus.sy.sy_maxaspd > sy_val) {
					sd->bonus.sy.sy_maxaspd = sy_val;
				}
			}
			break;
		case SP_SY_MAXASPD2:
			if (sd->state.lr_flag != 2)
				sd->bonus.sy.sy_maxaspd2 -= 10 * val;
			break;
...
bool pc_steal_item(map_session_data *sd,struct block_list *bl, uint16 skill_lv)
{
	...
	// base skill success chance (percentual)	基本技能成功几率（百分比）
...
enum e_setpos pc_setpos(map_session_data* sd, unsigned short mapindex, int x, int y, clr_type clrtype)
{
	...
	if (sd && sd->state.changemap && !sd->state.connect_new) {
		if (sd->bl.m != 0) {
			pc_setreg(sd, add_str("@frommap_id"), sd->bl.m);
			pc_setreg(sd, add_str("@frommap_x"), sd->bl.x);
			pc_setreg(sd, add_str("@frommap_y"), sd->bl.y);
			pc_setregstr(sd, add_str("@frommap_name$"), map[sd->bl.m].name);
		}
		else {
...
#else
int pc_dead(map_session_data *sd,struct block_list *src, uint16 skill_id)
...
/*==========================================
 * Read '@type' variables (temporary numeric char reg)读取'@type'变量（临时数字字符reg）
 *------------------------------------------*/
...
/*==========================================
 * Set '@type' variables (temporary numeric char reg)设置“@type”变量（临时数字字符reg）
 *------------------------------------------*/
...
/*==========================================
 * Read '@type$' variables (temporary string char reg)读取“@type$”变量（临时字符串char-reg）
 *------------------------------------------*/
...
/*==========================================
 * Set '@type$' variables (temporary string char reg)设置“@type$”变量（临时字符串char-reg）
 *------------------------------------------*/
...
#else
bool pc_equipitem(map_session_data *sd,short n,int req_pos,bool equipswitch, bool swapping)
 ...
 * @return max_param
 */
pec_ushort pc_maxparameter(map_session_data *sd, e_params param) {
	nullpo_retr(0, sd);
