...
public:
	...
	//-------761355061-----新增bonus结构---------------------
	struct sy_bonus {
		int sy_maxaspd;					//最大攻速上限(不可叠加,取最高值)
		int sy_maxaspd2;				//最大攻速上限(可叠加)
	...
	struct s_bonus {
		...
		//---------761355061----------在s_bonus中新增成员：sy_bonus
		sy_bonus sy;
		//-------------------------------------
		// 
...
#else
bool pc_equipitem(map_session_data *sd, short n, int req_pos, bool equipswitch = false, bool swapping = false);
