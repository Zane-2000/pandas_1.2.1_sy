...
uint64 PetDatabase::parseBodyNode( const ryml::NodeRef& node ){
	...
	if( this->nodeExists( node, "Fullness" ) ){
		...
		if( !this->asInt16( node, "Fullness", fullness ) ){
			return 0;
		}
...
int pet_menu(map_session_data *sd,int menunum)
{
	...
	*/
	{
		pc_setreg(sd, add_str("@pet_menunum"), menunum);						//菜单选项序号0~4
		pc_setreg(sd, add_str("@pet_mobid"), sd->pd->pet.class_);				//宠物魔物id
		pc_setreg(sd, add_str("@pet_gid"), sd->pd->bl.id);						//宠物游戏ID
		pc_setreg(sd, add_str("@pet_intimate"), sd->pd->pet.intimate);			//宠物亲密度
		pc_setreg(sd, add_str("@pet_hungry"), sd->pd->pet.hungry);				//饱食度

		pc_setreg(sd, add_str("@pet_map_id"), sd->pd->bl.m);					//宠物地图ID
		pc_setreg(sd, add_str("@pet_map_x"), sd->pd->bl.x);						//宠物X坐标
		pc_setreg(sd, add_str("@pet_map_y"), sd->pd->bl.y);						//宠物Y坐标

		if (npc_script_filter(sd, NPCF_SY_PET_MENU )) {
			return 1;
		}
